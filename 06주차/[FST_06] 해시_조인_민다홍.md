...
| -------------------------------------------------------------- | ------------------------------ | ----------------------------------------------------- |
| **소량 데이터 조인**                                                  | **Nested Loop Join (NL 조인)**   | 인덱스 기반 랜덤 액세스에 유리함. 소량 데이터일수록 효율적.                    |
| **대량 데이터 조인**                                                  | **Hash Join (해시 조인)**          | 인덱스 접근보다 메모리 기반 해시 탐색이 유리.                            |
| **대량 데이터 조인인데 해시 조인 불가** <br> (등치 조건이 아님 / 조인 조건 없음, 즉 카테시안 곱) | **Sort Merge Join (소트 머지 조인)** | 해시 조인은 등치(=) 조인만 가능하므로, 비등치(<, >, BETWEEN 등) 조인 시 사용. |
...
: Lateral로 선언하면,인라인 뷰에서 메인쿼리 테이블의 컬럼을 참조 가능
   세가지 기존의 구문으로 원하는 실행계획을 사용할 수 있기 때문에 PlusDown 기능이 잘 작동하지 않을떄만 사용하는 것을 추천 
   -> Lateral 인라인 뷰
...
사용자 함수 호출 시도 스칼라 서브쿼리 형태로 작성하면 캐싱 효과 적용 가능하다.
...
<img src="dahong/plusdown.png" width="600">
